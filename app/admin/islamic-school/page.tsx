...
export default function IslamicSchoolPageEditor() {
  ...
  useEffect(() => {
    const savedTab = localStorage.getItem("activeTab-islamic-school");
    if (savedTab) {
  ...
  const handleSave = async (sectionId: string) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        localStorage.setItem("activeTab-islamic-school", activeTab);
            ...
            <nav className="inline-flex min-w-max scroll-px-4 px-8 py-2 md:px-0" aria-label="Tabs">
              {tabs.map((tab) => (
                <button
                  ...
                  onClick={() => {
                    setActiveTab(tab.id);
                    localStorage.setItem("activeTab-islamic-school", tab.id);
                  }}
                  className={`
