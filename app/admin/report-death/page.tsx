...
export default function ReportDeathPageEditor() {
  ...
  useEffect(() => {
    const savedTab = localStorage.getItem("activeTab-report-death");
    if (savedTab) {
  ...
  const handleSave = async (sectionId: string) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        localStorage.setItem("activeTab-report-death", activeTab);
            ...
            <nav className="inline-flex min-w-max scroll-px-4 px-8 py-2 md:px-0" aria-label="Tabs">
              {tabs.map((tab) => (
                <button
                  ...
                  onClick={() => {
                    setActiveTab(tab.id);
                    localStorage.setItem("activeTab-report-death", tab.id);
                  }}
                  className={`
