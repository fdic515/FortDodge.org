...
export default function ElectionsNominationsPageEditor() {
  ...
  useEffect(() => {
    const savedTab = localStorage.getItem("activeTab-elections-nominations");
    if (savedTab) {
  ...
  const handleSave = async (sectionId: string) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        localStorage.setItem("activeTab-elections-nominations", activeTab);
            ...
            <nav className="inline-flex min-w-max scroll-px-4 px-8 py-2 md:px-0" aria-label="Tabs">
              {tabs.map((tab) => (
                <button
                  ...
                  onClick={() => {
                    setActiveTab(tab.id);
                    localStorage.setItem("activeTab-elections-nominations", tab.id);
                  }}
                  className={`
